...
This completes the deployment process. Remember to keep your server updated (`sudo apt update && sudo apt upgrade -y`) and monitor logs (`sudo journalctl -u mermaid-app`, `/var/log/nginx/access.log`, `/var/log/nginx/error.log`) if issues arise.
